...
const traceEventsStr = await evaluate(
  cdp,
  `window.__GEMINI_TRACE__ ? window.__GEMINI_TRACE__.dump() : "[]"`,
);
...
for (const turnId of Object.keys(turns)) {
  ...
  if (
    (t["USER_TURN_START"] && t["PLAYBACK_STARTED"]) ||
    t["FIRST_RESPONSE_AUDIO_BYTE"] ||
    t["FIRST_RESPONSE_AUDIO_CHUNK"] ||
    t["PLAYBACK_SCHEDULED"]
  ) {
    const e2e =
      (t["PLAYBACK_STARTED"] ||
        t["PLAYBACK_SCHEDULED"] ||
        t["FIRST_RESPONSE_AUDIO_BYTE"] ||
        t["FIRST_RESPONSE_AUDIO_CHUNK"]) - t["USER_TURN_START"];
    const cognition = t["COGNITION_COMPLETE"]
      ? t["COGNITION_COMPLETE"] - t["COGNITION_START"]
      : null;
    const decision = t["DECISION_COMPLETE"] ? t["DECISION_COMPLETE"] - t["DECISION_START"] : null;
    const geminiTtfb = t["FIRST_RESPONSE_AUDIO_BYTE"]
      ? t["FIRST_RESPONSE_AUDIO_BYTE"] - t["GEMINI_REQUEST_SENT"]
      : null;
