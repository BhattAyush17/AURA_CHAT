...
function simulateConversation(
  ...
  for (let i = 0; i < turns; i++) {
    let text = "";
    if (language === "english")
      text = "this is a very clear english sentence that I am speaking right now today";
...
function runTests() {
  ...
  console.log(
    `Mathematical verification: (1-alpha)^20 = ${Math.pow(1 - alpha, 20).toFixed(4)} (should be ≈0.5000)\n`,
  );
  ...
  console.log(
    "One 50-turn conversation MUST have the same longitudinal weight as ONE 1-turn conversation.\n",
  );
  ...
  console.log(
    `  After 1x50-turn Hindi conv: English Ratio ${after_50turn.language.value.englishRatio.toFixed(4)} (Δ=${influence_50turn.toFixed(4)}, convs=${convs_50turn})`,
  );
  console.log(
    `  After 1x1-turn Hindi conv:  English Ratio ${after_1turn.language.value.englishRatio.toFixed(4)} (Δ=${influence_1turn.toFixed(4)})`,
  );
  console.log(`  Both should increment totalConversationsAnalyzed by exactly 1.`);
  console.log(
    `  PASS: ${Math.abs(convs_50turn - baseline_convs) === 1 ? "YES" : "NO"} — 50-turn conv counted as exactly 1 conversation.`,
  );
  ...
  console.log(
    `  Clustering Ratio: 1-conv influence / 50-conv influence = ${(influence_50turn / influence_50indep).toFixed(3)}`,
  );
  console.log(
    `  PASS: ${influence_50turn < influence_50indep * 0.2 ? "YES" : "NO"} — 50-turn conv influence < 20% of 50 independent convs.\n`,
  );
  ...
  console.log(
    "After 20 contradictory independent conversations, old baseline influence should be ≈50%.\n",
  );
  ...
  console.log(
    `  PASS: ${c_after.language.value.primary === "english" ? "YES" : "NO"} — baseline remains English.\n`,
  );
  ...
  for (const stage of stages) {
    ...
    console.log(
      `  After ${stage} Hindi convs: English ${dp.language.value.englishRatio.toFixed(4)} | State: ${dp.language.state} | Confidence: ${dp.language.confidence.toFixed(2)}`,
    );
  }
  console.log("");
  ...
  console.log(
    `  Confidence: ${ep.language.confidence.toFixed(2)} | State: ${ep.language.state} | Variance: ${(ep.language.variance ?? 0).toFixed(3)}`,
  );
  console.log(
    `  PASS: ${ep.language.state === "CONFLICTING" || ep.language.state === "RECENTLY_CHANGED" ? "YES" : "NO"} — not falsely KNOWN.`,
  );
  console.log(
    `  PASS: ${ep.language.confidence < 0.6 ? "YES" : "NO"} — confidence penalized by variance.\n`,
  );
  ...
  console.log(
    `  Global: ${fp.language.value.primary} (English Ratio: ${fp.language.value.englishRatio.toFixed(3)})`,
  );
  console.log(
    `  Casual: ${fp.contextualLanguage.casual.value.primary} (English Ratio: ${fp.contextualLanguage.casual.value.englishRatio.toFixed(3)})`,
  );
  console.log(
    `  Technical: ${fp.contextualLanguage.technical.value.primary} (English Ratio: ${fp.contextualLanguage.technical.value.englishRatio.toFixed(3)})`,
  );
  ...
  console.log(
    `  PASS: ${g_state_after1 !== "RECENTLY_CHANGED" ? "YES" : "NO"} — 1 anomaly does not trigger RECENTLY_CHANGED.`,
  );
  ...
  console.log(
    `  PASS: ${g_state_after10 === "RECENTLY_CHANGED" || g_state_after10 === "CONFLICTING" ? "YES" : "NO"} — sustained change detected.\n`,
  );
  ...
  console.log(
    `  Saved convs: ${savedConvs} → Loaded convs: ${loadedProfile.totalConversationsAnalyzed}`,
  );
  console.log(
    `  Saved confidence: ${savedConfidence.toFixed(3)} → Loaded confidence: ${loadedProfile.language.confidence.toFixed(3)}`,
  );
  console.log(
    `  Saved explicit prefs: ${savedExplicitPrefs} → Loaded explicit prefs: ${loadedProfile.explicitPreferences.length}`,
  );
  console.log(`  Current turn signal after reload: ${h2.getCurrentTurnSignal()}`);
  console.log(
    `  PASS: ${loadedProfile.totalConversationsAnalyzed === savedConvs ? "YES" : "NO"} — longitudinal state survives reload.`,
  );
  console.log(
    `  PASS: ${h2.getCurrentTurnSignal() === null ? "YES" : "NO"} — turn signal does NOT survive reload.\n`,
  );
