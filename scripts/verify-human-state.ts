...
function runTests() {
  ...
  assert(
    ...
    "Test 1: Neutral baseline produces no fabricated hypotheses.",
  );
  ...
  assert(
    state.affective.arousal.estimate > 0,
    "Test 2: Increased activation produces increased arousal hypothesis.",
  );
  assert(
    state.affective.hypotheses.some((h) => h.type.includes("arousal")),
    "Test 2: Contains arousal hypothesis.",
  );
  ...
  assert(
    state.affective.hypotheses.some((h) => h.type.includes("tension")),
    "Test 3: Contradiction produces tension/uncertainty hypothesis.",
  );
  assert(
    state.affective.hypotheses.some((h) => h.contradictingEvidence.length > 0),
    "Test 3: Contradiction preserves contradicting evidence.",
  );
  ...
  assert(
    hyp2.confidence > hyp1.confidence,
    "Test 4: Temporal persistence increases confidence appropriately.",
  );
  ...
  assert(
    state3.affective.arousal.estimate <= oldArousal,
    "Test 5: Recovery - state decays when evidence disappears.",
  );
  ...
  assert(
    state.affective.hypotheses.length === 0,
    "Test 6: Missing modality does not create negative affective evidence.",
  );
  ...
  assert(
    state.affective.hypotheses.length === 0 && state.affective.arousal.estimate === 0,
    "Test 10: Empty evidence keeps Human State uncertain and does not fabricate a state.",
  );
