...
function attachCollectors(page) {
  ...
  page.on("console", (m) => {
    ...
    if (m.type() === "warning" && /CORS|cross-origin/i.test(m.text()))
      errs.cors.push(m.text().slice(0, 300));
  });
...
async function mobileSection() {
  ...
  report.sections.mobile = {
    layout,
    consoleErrors: errs.console.length,
...
async function voiceSection() {
  ...
  const sessionLog = [];
  page.on("console", (m) => {
    if (/Starting session|Ending session|Watchdog|\[AURA\]/.test(m.text()))
      sessionLog.push(m.text().slice(0, 100));
  });
  await mic.click();
  const transitions = [];
  ...
  report.sections.voice = {
    ...
    errors: { console: errs.console, page: errs.page },
  };
  await browser.close();
}
...
async function a11ySection() {
  ...
  const a11y = await page.evaluate(() => {
    ...
    return {
      buttonsWithoutAriaOrText: buttons.filter(
...
console.log(
  "PERF:",
  JSON.stringify(
    report.sections.performance.map((p) => ({
      ttfb: p.ttfb,
      dcl: p.dcl,
      load: p.load,
      heap: p.heapMB + "MB",
      errs: p.consoleErrors,
    })),
    null,
    1,
  ),
...
console.log(
  "SOAK heap:",
  report.sections.soak.heartbeats.map((h) => h.heapMB + "MB").join(" -> "),
  "| console errs:",
  report.sections.soak.errors.console.length,
  "| http errs:",
  report.sections.soak.errors.http4xx5xx.length,
);
console.log("NETWORK:", JSON.stringify(report.sections.network));
console.log("MOBILE:", JSON.stringify(report.sections.mobile));
console.log(
  "VOICE:",
  JSON.stringify(report.sections.voice.statusTransitions),
  "| errs:",
  report.sections.voice.errors.console.length,
);
