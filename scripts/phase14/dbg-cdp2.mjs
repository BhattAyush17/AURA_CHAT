...
try {
  new Function(inject);
  console.log("syntax OK, length:", inject.length);
} catch (e) {
...
const page =
  ctx.pages().find((p) => p.url().startsWith("http://127.0.0.1:5173")) ?? (await ctx.newPage());
await page.addInitScript(inject);
await page.reload({ waitUntil: "load" });
await page.waitForTimeout(4000);
const r = await page.evaluate(() => ({
  rc2: typeof window.__rc2,
  sync: typeof window.__rc2Sync,
  sr: typeof window.__rc2WrappedSR,
  tts: typeof window.speechSynthesis.__rc2wrapped,
  brain: localStorage.getItem("aura_active_brain"),
  key: (sessionStorage.getItem("openrouter_api_key") || "").slice(0, 8),
}));
