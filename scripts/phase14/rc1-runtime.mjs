...
function collectors(page) {
  ...
  page.on("console", (m) => {
    if (m.type() === "error") e.console.push(m.text().slice(0, 200));
  });
  page.on("pageerror", (err) => e.page.push(String(err).slice(0, 200)));
  page.on("response", (r) => {
    if (r.status() >= 400) e.http.push(`${r.status()} ${r.url().slice(0, 120)}`);
  });
  return e;
}
...
async function main() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const timing = await page.evaluate(() => {
        ...
        return {
          ...
          transfer: Math.round(nav.transferSize / 1024) + "KB",
          ...paints,
        };
      });
      ...
      const metrics = await cdp
        ...
        .then((m) => Object.fromEntries(m.metrics.map((x) => [x.name, Math.round(x.value)])));
      out.push({
        timing,
        heapMB,
        fps,
        scriptMs: metrics.ScriptDuration,
        taskMs: metrics.TaskDuration,
        nodes: metrics.Nodes,
        errConsole: e.console.length,
        errPage: e.page.length,
      });
      await page.close();
    }
    await browser.close();
  ...
  await section("reload", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      heaps.push(
        await page.evaluate(() => Math.round(performance.memory.usedJSHeapSize / 1024 / 1024)),
      );
    }
    await browser.close();
    report.sections.reload = {
      heaps,
      errConsole: e.console.length,
      errPage: e.page.length,
      errHttp: e.http.length,
    };
  });
  ...
  await section("lifecycle", async () => {
    ...
    try {
      ...
      freeze = "unsupported(headless-shell)";
    }
    const heapMB = await page.evaluate(() =>
      Math.round(performance.memory.usedJSHeapSize / 1024 / 1024),
    );
  ...
  await section("micDenied", async () => {
    ...
    await browser.close();
    report.sections.micDenied = {
      pageErrors: e.page.length,
      hintOrErrorShown: /key|mic|permission|allow/i.test(body),
      body,
    };
  });
  ...
  await section("invalidKey", async () => {
    const browser = await chromium.launch({ headless: true, args: FAKE });
    const page = await loadPage(browser, {
      key: "sk-or-v1-invalidkeyinvalidkeyinvalidkeyinvalidkey00",
    });
    const e = collectors(page);
    ...
    await browser.close();
    report.sections.invalidKey = {
      pageErrors: e.page.length,
      consoleErrors: e.console.slice(0, 2),
      body,
    };
  });
  ...
  await section("sileroFail", async () => {
    ...
    await browser.close();
    report.sections.sileroFail = {
      fallbackSeen,
      relevantLogs: logs.filter((l) => /Silero|VAD|sarvam/i.test(l)).slice(0, 5),
    };
  });
  ...
  await section("offline", async () => {
    ...
    await page.waitForTimeout(4000);
    await browser.close();
    report.sections.offline = {
      errorsWhileOffline: errsOffline,
      errorsAfterReconnect: e.page.length + e.console.length - errsOffline,
      alive: "complete",
    };
  });
  ...
  await section("mobile", async () => {
    ...
    const l = await landscape.evaluate(() => ({
      w: innerWidth,
      h: innerHeight,
      hScroll: document.documentElement.scrollWidth > innerWidth,
    }));
    ...
    const scrollable = await portrait.evaluate(
    ...
    report.sections.mobile = {
      ...
      scrollable,
      micTapOk: true,
      pageErrors: pcol.page.length + lcol.page.length,
    };
  });
  ...
  await section("a11y", async () => {
    ...
    const focusable = await page.evaluate(
      () => document.querySelectorAll("button, a[href], input, [tabindex]").length,
    );
    await browser.close();
  ...
  await section("firefox", async () => {
    ...
    await browser.close();
    report.sections.firefox = {
      ...state,
      pageErrors: e.page.length,
      consoleErrors: e.console.slice(0, 3),
    };
  });
  ...
  console.log(
    "RELOAD heaps:",
    report.sections.reload?.heaps?.join(" "),
    "| errs:",
    JSON.stringify({
      c: report.sections.reload?.errConsole,
      p: report.sections.reload?.errPage,
      h: report.sections.reload?.errHttp,
    }),
  );
