...
Nobody else touches the microphone, AudioContext, playback graph, or VAD. Ever. Output providers own the _queue/epoch/cancellation_; Media owns the _physical playback graph_. The only shared data flows are explicit read-only streams.
...
Tier A (high-end, battery healthy): warm transport + AudioContext + STT + speculative. Tier B (mid): transport only. Tier C (battery saver): cold path only. Network quality degrades _deadlines_; tiers select _which_ resources warm.
...
Warmup attributes (deadlines/vector/debt) ship _inside_ each phase — they are per-runtime attributes, not a phase. Warm-path tuning is telemetry-gated (Phase 6) and reverted on negative ROI per device tier.

---

## 10. The Shift That Made This Stable

Originally: _browser speech APIs are the architecture._
