...
2. **Buffer Destruction (Slicing)**: The `fullText` variable was functioning purely as a text buffer for the sentence boundary regex parser (`SENTENCE_END.exec(fullText)`). To handle streaming properly, the parser slices `fullText` once a sentence is found and passed to the TTS queue (`fullText.slice(lastIndex)`). Since `setWords(fullText)` was called *after* accumulating but *before* slicing, it correctly flashed text to the UI, but it never preserved the *entire* generated response.
