...
**The cognitive block DOES reach the LLM on both providers — but the cognitive *decision chain* is
...
3. 🔴 **LLM reliability:** `deepseek-chat` frequently answers a *different question* than asked, hallucinates,
...
4. 🟡 **Hostile low-warmth behavior:** injecting `warmth:0.2 + venting + Challenge` produced sarcastic,
   combative replies to a clearly frustrated user — the attention parameters *do* steer behavior, but the
...
**Provider parity:** Both providers send the *identical* backend payload — the LLM stage is byte-identical.
...
| G fatigue | "answer like last ten…" | low initiative/direct | "*tired sigh* …what's on your mind?" | 1.5s | 🟢 mostly |
...
- The only behavioral difference that would *appear* to affect cognition: Sarvam Path B adds `[ADAPTIVE
...
*(Report only — no changes made. Every item is backed by evidence above.)*
...
*No production code modified; temp scripts removed; git tree unchanged (flat 220 pre-existing); no commits.*
