...
| **Useful**       | ✅ Partial but real: `hasPersonalHistory` moves turn-5 from ACQUAINTING to COMFORTABLE (`RegisterState.ts:81`), unlocking CASUAL/PLAYFUL/INTIMATE registers — memory _changes the response_ via the register gate.           |
| **Ignored**      | ⚠️ The policy can only _decide_ Ignore — see Finding 1.                                                                                                                                                                      |
...
1. **The Executive's memory decision layer is dead code in production.** The live context is
   ...
   reason _"no memory clears relevance threshold"_. The intended gate
...
- The memory decision is structured as _advisory_ (`memoryPolicy` in the plan) rather than
  _enforced_ on the injection channels — so even a working policy could not have prevented
  injection.
...
decide _usage_ — is disconnected: it sees empty scores, can only decide Ignore, and would
