...
Adaptive Attention, OpenRouter/Sarvam/Gemini provider *contracts*, YouTube/yt-dlp, playback state, identity model.
**Git policy:** no `add`/`commit`/`reset`/`push`. Pre-existing dirty tree = **220 modified tracked files**; stays
exactly **220** (verified). Only new *untracked* source added.
...
- **Gemini Live** inherently can't fold a *per-turn* decision into a one-shot session instruction; it receives the
