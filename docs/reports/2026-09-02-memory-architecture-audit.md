...
Line 1072 sits *before* the `try {` at `:1097`, so the `TypeError` escapes `processTurn`; call sites `:2018`/`:2269` are bare `await` with no catch and there is no `unhandledrejection` handler.
