...
| `npm run build:dev`            | ✅ PASS 12.08s (one transient rollup failure on a _parallel_ first attempt — not reproducible; prod also built)                         |
...
matrix of _provider-specific_ stages:
...
**Conclusion:** Sarvam is _not_ a real second pipeline — it is OpenRouter-LLM + sarVam STT/TTS that are
...
- **Conclusion:** No measured cache-pollution of unrelated turns (F10). The cache _gaps_ are: uncached
...
- **Move fully off the block:** you **cannot** emit metadata and still inject the grounding into the _same
  turn's_ system prompt without waiting — the LLM call follows immediately and needs `atmosphere_grounding`
...
_Full evidence in this report; temp measurement scripts removed; no source files modified; git tree
unchanged from its pre-existing 220-file dirty state (only the new report file added, untracked)._
