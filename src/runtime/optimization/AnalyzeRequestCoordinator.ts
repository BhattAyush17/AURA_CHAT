...
export class AnalyzeRequestCoordinator {
   ...
   */
  public dispatchSpeculative(
    transcript: string,
    fetcher: () => Promise<BehaviorAnalysis>,
  ...
  public getFinalAnalysis(
    ...
    if (
      this.activePromise &&
      finalTranscript.startsWith(this.activeTranscript) &&
      this.activeTranscript.length / finalTranscript.length > 0.7
    ) {
      this.metrics.hitRate++;
      return this.activePromise;
    }
