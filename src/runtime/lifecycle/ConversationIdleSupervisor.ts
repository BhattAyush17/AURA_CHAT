...
export class ConversationIdleSupervisor {
  ...
  public ping() {
    ...
    if (this.state !== IdleState.ACTIVE) {
      RuntimeTelemetry.getInstance().logEvent({
        subsystem: "IdleSupervisor",
        severity: "info",
        data: { event: "IdleCancelled" },
      });
  ...
  private checkState(activity: RuntimeActivityState) {
    ...
    if (
      this.state === IdleState.IDLE_WARNING &&
      idleTime > this.policy.warningThresholdMs + this.policy.terminationThresholdMs
    ) {
      ...
      RuntimeTelemetry.getInstance().logEvent({
        ...
        severity: "info",
        duration: idleTime,
        data: { event: "ConversationAutoClosed" },
      });
