...
const LLM_QUALITY = {
  openrouter_haiku: 1.0,
  openrouter_mini: 0.85,
  openrouter_llama: 0.7,
  gemini_direct: 0.6,
  degraded: 0.1,
};
...
const VOICE_QUALITY = {
  ...
  webspeech_webspeech: 0.7,
  textinput_sarvam: 0.6,
  textinput_textonly: 0.0,
};
...
export async function validateAllPipelines(): Promise<ValidatorReport> {
  ...
  const addResult = (
    ...
    const isAvail =
      (memory !== "gemini" || state.gemini_key_exists) &&
    ...
    const overall = 0.4 * memQual + 0.35 * llmQual + 0.25 * voiceQual;
    ...
    results.push({
      ...
      quality_scores: {
        memory_quality: memQual,
        llm_quality: llmQual,
        voice_quality: voiceQual,
  ...
  addResult(
    "A1X",
    "Supabase+Gemini + OR Haiku + Sarvam",
    "gemini",
    "openrouter_haiku",
    "sarvam",
    "sarvam",
  ...
  addResult(
    "B1X",
    "Supabase+FastEmbed + OR Haiku + Sarvam",
    "fastembed",
    "openrouter_haiku",
    "sarvam",
    "sarvam",
  ...
  addResult(
    "C1Y",
    "Supabase+FTS + OR Haiku + WebSpeech",
    "fts",
    "openrouter_haiku",
    "webspeech",
    "webspeech",
  ...
  addResult(
    "C2Y",
    "Postgres FTS + OpenRouter Mini + Web Speech",
    "fts",
    "openrouter_mini",
    "webspeech",
    "webspeech",
  ...
  addResult(
    "D3Z",
    "Local Browser + Gemini Direct + Text Only",
    "tagmatch",
    "gemini_direct",
    "textinput",
    "textonly",
  ...
  addResult(
    "D2Z",
    "Local Browser + OR Mini + Text Only",
    "tagmatch",
    "openrouter_mini",
    "textinput",
    "textonly",
  ...
  const bestAvail =
    results
      ...
      .sort((a, b) => b.quality_scores.overall - a.quality_scores.overall)[0]?.combo_id || "D3Z";
  const bestSpeed =
    results.filter((r) => r.available).sort((a, b) => a.latency.total_ms - b.latency.total_ms)[0]
      ?.combo_id || "D3Z";
  const bestQual =
    results.sort((a, b) => b.quality_scores.overall - a.quality_scores.overall)[0]?.combo_id ||
    "A1X";
  ...
  return {
    ...
    summary: {
      ...
      pipeline_health: {
        ...
        overall:
          state.supabase_connected && state.openrouter_key_exists && state.sarvam_available
            ? "healthy"
            : "degraded",
      },
    },
