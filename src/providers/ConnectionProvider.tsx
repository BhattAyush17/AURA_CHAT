...
export const ConnectionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const initializeConnection = async () => {
      ...
      const supabase_connected =
        healthData.status === "fulfilled" &&
        (healthData.value?.checks?.supabase?.ok || healthData.value?.supabase_connected);
    ...
    const pipelineSelector = () => {
      ...
      const llm_quality =
        active_llm === "openrouter_haiku"
          ? 1.0
          : active_llm === "openrouter_mini"
            ? 0.85
            : active_llm === "openrouter_llama"
              ? 0.7
              : active_llm === "gemini_direct"
                ? 0.6
                : 0.1;
      const voice_quality =
        active_voice_in === "sarvam" && active_voice_out === "sarvam"
          ? 1.0
          : active_voice_out === "sarvam" && active_voice_in === "webspeech"
            ? 0.85
            : active_voice_in === "webspeech" && active_voice_out === "webspeech"
              ? 0.7
              : active_voice_in === "textinput" && active_voice_out === "sarvam"
                ? 0.6
                : 0.0;
      const overall_score = 0.4 * memory_quality + 0.35 * llm_quality + 0.25 * voice_quality;
