...
export function RedisManager() {
  ...
  return (
    <div className="w-full space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
      ...
      {loading && !stats ? (
        <div className="p-4 text-muted-foreground text-xs text-center animate-pulse bg-background border border-border rounded-lg">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-background p-4 rounded-lg border border-border">
            ...
            <p className="text-[11px] text-muted-foreground mb-3 leading-relaxed">
              Stores raw audio transcripts and conversation events across all active sessions.
              Unbounded streams consume memory.
            </p>
            <button
              onClick={clearStream}
          ...
          <div className="bg-background p-4 rounded-lg border border-border">
            ...
            {stats.active_sessions?.length === 0 ? (
              <div className="text-xs text-muted-foreground text-center py-4 border border-dashed border-border/50 rounded">
              ...
              <ul className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-1">
                {stats.active_sessions?.map((session: any) => (
                  <li
                    key={session.id}
                    className="flex justify-between items-center bg-muted/30 p-2.5 rounded border border-border/50"
                  >
                    <div>
                      <div className="font-mono text-[11px] text-foreground">
                        {session.id.split("__")[0]}
                      </div>
